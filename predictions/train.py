...
def remove_additional_columns(df):
    return df[['GRE Score','TOEFL Score', 'CGPA', 'University Rating', 'Chance of Admit ']]

def get_linear_regressor(df):
    feature = df['Chance of Admit ']
    labels = df.drop('Chance of Admit ', axis=1)
    x_train, x_test, y_train, y_test = train_test_split(labels, feature, test_size=0.10)
...
def get_rf_regressor(df):
    ...
    labels = df.drop('Chance of Admit ', axis=1)
    x_train, x_test, y_train, y_test = train_test_split(labels, feature, test_size=0.10)
    r = RandomForestRegressor(n_estimators = 100,random_state = 42,criterion="mse")
    r = r.fit(x_train, y_train)
    return r
...
if __name__ == '__main__':
    ...
    user_dictionary = {'GRE Score':[325],'TOEFL Score':[112], 'CGPA':[8.0], 'University Rating': [4]}
